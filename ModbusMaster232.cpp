...
/*
 ...
 - Function  makeWord
  ...
  Modified by Thellyson Francisco 2020
  Date: 2020/03/04
  //20200304_1 Insert Tx enable
...
@ingroup setup
*/
ModbusMaster232::ModbusMaster232(uint8_t u8MBSlave, unsigned char txEnablePin)
{
  _u8SerialPort = 0;
  _u8MBSlave = u8MBSlave;
  _txEnablePin = txEnablePin;                      //20200304_1
  pinMode(txEnablePin, OUTPUT);					   //20200304_1
}
...
Sets up the Serial2 port using specified baud rate.
Call once class has been instantiated, typically within setup().
...
uint8_t ModbusMaster232::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  // transmit request
  digitalWrite(_txEnablePin, HIGH);             //20200304_1
  for (i = 0; i < u8ModbusADUSize; i++)
  {
		Serial2.print(char(u8ModbusADU[i]));
  }
  ...
  digitalWrite(_txEnablePin, LOW);             //20200304_1
  ...
  {    
		u8ModbusADU[u8ModbusADUSize] = Serial2.read();
		u8BytesLeft--;
		u8ModbusADUSize ++;
